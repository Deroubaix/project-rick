    ...
    {character && (
      <Container className="mt-4" style={{ backgroundColor: "#282c34", paddingTop: "50px", minHeight: '100vh' }}>
        <Row>
          ...
          <Col md={8} className="text-light" style={{ backgroundColor: "#282c34", overflow: "auto", maxHeight: "calc(100vh - 70px)" }}>
            ...
            {Array.isArray(episodes) && (
              <div className={`row row-cols-${episodes.length === 1 ? "1" : "2"}`}>
                {episodes.map((episode, index) => (
                  <div key={episode.id} className={`col${episodes.length === 1 ? "-12" : "-md-6"} mb-3`}>
                    <div className="text-light episode-container">
                      <p style={{  color: 'white', fontSize: '1.2rem', fontWeight: '800' }}>{episode.name}</p>
